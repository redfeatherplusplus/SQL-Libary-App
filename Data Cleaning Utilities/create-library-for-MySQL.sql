...
DELIMITER //
CREATE TRIGGER book_check_out_available AFTER INSERT ON BOOK_LOANS FOR EACH ROW
BEGIN
  IF ISNULL(NEW.Date_in) THEN
    UPDATE BOOK BK
	SET BK.Available = FALSE
...
DELIMITER //
CREATE TRIGGER book_check_in_available AFTER UPDATE ON BOOK_LOANS FOR EACH ROW
BEGIN
  IF NOT ISNULL(NEW.Date_in) THEN
    UPDATE BOOK BK
	SET BK.Available = TRUE
	WHERE BK.ISBN = NEW.ISBN;
  END IF;
END //
DELIMITER ;
